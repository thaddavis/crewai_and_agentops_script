...
Let's start with the manual approach, which is not the recommended way to do things, but it great for learning purposes...
Here on CrewAI's documentation they provide an outline of what a CrewAI app looks like so let's copy the general outline...
https://docs.crewai.com/getting-started/Start-a-New-CrewAI-Project-Template-Method/#creating-a-new-project

In the same way that a chef can look at a recipe and get the gist of what it means and improvise cooking the dish
We as coding people can look at code to understand what it's trying to do and can improvise on it just the same

First let's create these files & folders...

```
mkdir -p src/simple_crew
touch src/simple_crew/main.py 
```

<!--
Often when looking at a coding project, you'll see a file called something along the lines of "main" to indicate that it's important somehow...

In our scenario, main.py means that running this file will run all the code in our project...

You can call files whatever you like for example `blah.py`

BUT just so it's easy for others to understand and build on top of our code we follow conventions
-->

```inside src/simple_crew/main.py let's place the following code
def run():
  print("Running a simple crew...")

if __name__ == "__main__":
  run()
```

TEST: `python src/simple_crew/main.py` √

OK! Now instead of simply printing a line that says "Running a simple crew...", let's do something more interesting...
